...
export default class DroneConfigParser {
  ...
  compileDefaults(module, moduleType) {
    if (moduleInfo.hasOwnProperty(moduleType)) {
      var m = moduleInfo[moduleType];

      // check inherited properties first, so we can override them if needed
      if (m.hasOwnProperty("inherits")) {
        this.compileDefaults(module, m.inherits[0]);
      }

      if (m.hasOwnProperty("default")) {
        // transfer defaults across to config
        m.default.forEach((def) => {
          var p = this.getParamByName(module, def.param);
          if (p) {
    ...
    if (moduleInfo.hasOwnProperty(moduleType)) {
      ...
      if (pName == "publish") {
        ...
        for (var i = 0; i < parts.length; i++) {
          ...
          if (!pubOrSubInfo) {
            res.error +=
              parts[i] + " is an unknown param for this module type; ";
          }
        }
      } else {
  ...
  parse(config) {
    ...
    for (const [key, obj] of Object.entries(this.config.modules)) {
      this.compileDefaults(obj, obj.type);
    }
